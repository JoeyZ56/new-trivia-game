...
const Trivia = () => {
  ...
  const fetchQuestions = async () => {
    try {
      const res = await fetch("https://jservice.io/api/category?id=20");
      ...
      {isGameOver ? (
        ...
        <>
          <div className="headerContainer">
            <h2 className="trivia-title">General Trivia</h2>
            <div className="trivia-score">{score}</div>
          </div>
          ...
          <div className=" flex justify-center mt-[80px]">
            <p className=" text-xl max-sm: text-center px-3">
              {triviaQuestion}
            </p>
          </div>
          {correctAnswer && (
